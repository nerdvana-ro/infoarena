...
function getFileExtension(path) {
  // https://stackoverflow.com/a/12900504/6022817
...
function Submit_AutoCompiler() {
  ...
  var ext = getFileExtension(f.val());
  if (ext == 'cc' || ext == 'c++') {
    ext = 'cpp';
  }

  var opt = compiler.find('option[value="' + ext + '"]');

  if (opt.length) {
    compiler.val(ext);
  } else {
    compiler.val('-');
    alert('Atenție! Pentru fișierul selectat nu am putut alege automat un compilator.');
  }
}
